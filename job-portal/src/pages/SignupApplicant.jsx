...
export default function SignupApplicant() {
    ...
    const onSubmit = async (e) => {
        ...
        setError('')
        setSubmitting(true)
        const res = await signupApplicant({ name, email, password })
        if (res.ok) {
            setCreated(true)
        } else {
            ...
            <div className="w-full max-w-xl relative">
                <div className="rounded-2xl bg-gradient-to-br from-zinc-900/90 via-zinc-900/70 to-zinc-900/50 p-[1px] shadow-2xl">
                    <div className="rounded-2xl bg-zinc-950/70 backdrop-blur-md p-6 sm:p-8">
                        <h2 className="text-2xl font-semibold text-white">Sign Up as Applicant</h2>
                        <p className="mt-1 text-sm text-zinc-400">Create your applicant account to apply for jobs</p>
                        <form onSubmit={onSubmit} className="mt-6">
                            ...
                            >
                                {submitting ? 'Creating...' : 'Create Applicant Account'}
                            </button>
                            {created && (
                                <div className="mt-3 text-sm text-green-400">Account created. You can log in as an applicant.</div>
                            )}
                        </form>
                    </div>
                </div>
